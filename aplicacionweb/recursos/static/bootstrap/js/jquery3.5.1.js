...
var plana = arr.flat? función (matriz) {
	volver arr.flat.call (matriz);
}: función (matriz) {
	...
	función DOMEval (código, nodo, doc) {
		...
		if (nodo) {
			para (i en preserveScriptAttributes) {
				...
				// `jQuery.globalEval` para que pueda simular un nodo que no contiene
...
jQuery.fn = jQuery.prototype = {
	...
	even: function () {
		devuelve this.pushStack (jQuery.grep (this, function (_elem, i) {
			retorno (i + 1)% 2;
		}));
	},
		...
		if ((opciones = argumentos [i])! = nulo) {
			...
			para (nombre en opciones) {
				...
				if (profundo && copy && (jQuery.isPlainObject (copy) ||
					...
					// Asegura el tipo apropiado para el valor fuente
...
jQuery.extend ({
	...
	// Evalúa un script en un contexto proporcionado; recurre a la global
	// si no se especifica
	globalEval: function (código, opciones, doc) {
		DOMEval (código, {nonce: opciones && options.nonce}, doc);
	},
...
var i,
	...
	booleans = "verificado | seleccionado | asíncrono | enfoque automático | reproducción automática | controles | diferir | deshabilitado | oculto |" +
		"ismap | loop | múltiple | abierto | solo lectura | requerido | alcance",
	...
	rtrim = nuevo RegExp ("^" + espacio en blanco + "+ | ((?: ^ | [^ \\\\]) (?: \\\\.) *)" +
		espacio en blanco + "+ $", "g"),
	...
	matchExpr = {
		...
		"needsContext": nuevo RegExp ("^" + espacio en blanco +
			"* [> + ~] |: (par | impar | eq | gt | lt | nth | primero | último) (?: \\ (" + espacios en blanco +
	...
	funescape = function (escape, nonHex) {
		...
		volver no hexadecimal?

			// Elimina el prefijo de barra diagonal inversa de una secuencia de escape no hexadecimal
			no hexadecimal:

			// Reemplace una secuencia de escape hexadecimal con el punto de código Unicode codificado
			// Soporte: IE <= 11 +
...
tratar {
	...
	// eslint-disable-next-line expresiones no no utilizadas
		...
		if (documentIsHTML) {
			...
			if (support.qsa &&
				...
				if (nodeType === 1 &&
					...
					// Prefije cada selector en la lista
...
/ **
 * Crear cachés de valor clave de tamaño limitado
 * @returns {function (string, object)} Devuelve los datos del objeto después de almacenarlos en sí mismo con
 * nombre de propiedad de la cadena (con sufijo de espacio) y (si el caché es mayor que Expr.cacheLength)
...
función createCache () {
	...
	función caché (clave, valor) {
		...
		// Use (tecla + "") para evitar la colisión con las propiedades nativas del prototipo (vea el Problema # 157)
		if (keys.push (key + "")> Expr.cacheLength) {

			// Solo guarda las entradas más recientes
...
/ **
 * Marque una función para uso especial de Sizzle
 ...
 * @param {Function} fn Pasó el elemento creado y devuelve un resultado booleano
 * /
función afirmar (fn) {
...
/ **
 * Comprueba el orden de los documentos de dos hermanos
 * @param {Elemento} a
 * @param {Elemento} b
...
función createDisabledPseudo (deshabilitado) {
	...
	// Conocido: deshabilitado falsos positivos: conjunto de campos [deshabilitado]> leyenda: enésimo tipo (n + 2): deshabilitar
	función de retorno (elem) {
		...
		if ("formulario" en elem) {
			...
			// Verifique la discapacidad heredada en elementos relevantes no deshabilitados:
			// * elementos asociados a formularios listados en un conjunto de campos deshabilitado
			// https://html.spec.whatwg.org/multipage/forms.html#category-listed
			// https://html.spec.whatwg.org/multipage/forms.html#concept-fe-disabled
			// * elementos de opción en un optgroup deshabilitado
			// https://html.spec.whatwg.org/multipage/forms.html#concept-option-disabled
			// Todos estos elementos tienen una propiedad "form".
			if (elem.parentNode && elem.disabled === false) {

				// Los elementos de opción difieren a un grupo de padres principal si está presente
				if ("etiqueta" en elem) {
				...
				// Soporte: IE 6 - 11
				// Use la propiedad de acceso directo isDisabled para verificar los antepasados ​​del conjunto de campos deshabilitados
		...
		// Intenta eliminar elementos que no se puedan deshabilitar antes de confiar en la propiedad deshabilitada.
		...
		// Los elementos restantes no son: enabled ni: disabled
		falso retorno;
	};
}
 ...
 * @returns {Element | Object | Boolean} El nodo de entrada si es aceptable; de ​​lo contrario, un valor falso
 * /
 ...
 * @returns {Boolean} Verdadero iff elem es un nodo XML no HTML
 * /
isXML = Sizzle.isXML = function (elem) {
	...
	// Soporte: IE <= 8
	// Asume HTML cuando documentElement aún no existe, como cargar iframes por dentro
...
setDocument = Sizzle.setDocument = function (nodo) {
	...
	// Acceder a documentos de iframe después de la descarga arroja errores de "permiso denegado" (jQuery # 13936)
	// Soporte: IE 11+, Edge 17-18+
	// IE / Edge a veces arroja un error de "Permiso denegado" cuando se compara estrictamente
	// dos documentos; Las comparaciones superficiales funcionan.
	// eslint-disable-next-line eqeqeq
	...
	// Safari 4 - 5 solamente, Opera <= 11.6 - 12.x solo
	// IE / Edge y los navegadores anteriores no admiten la pseudoclase: scope.
	// Soporte: solo Safari 6.0
	// Safari 6.0 es compatible con: scope pero es un alias de: root allí.
	support.scope = afirmar (función (el) {
		docElem.appendChild (el) .appendChild (document.createElement ("div"));
		return typeof el.querySelectorAll! == "undefined" &&
			! el.querySelectorAll (": alcance fieldset div") .length;
	});
	...
	support.attributes = afirmar (función (el) {
		el.className = "i";
	...
	// Comprueba si getElementsByTagName ("*") solo devuelve elementos
	...
	// Los métodos getElementById rotos no captan nombres establecidos programáticamente,
	...
	Expr.find ["TAG"] = support.getElementsByTagName?
		...
		función (etiqueta, contexto) {
			...
			// Filtra los posibles comentarios
			if (etiqueta === "*") {
	...
	/ * QSA / coincidenciasSelector
	-------------------------------------------------- -------------------- * /

	// Soporte QSA y MatchSelector

	// partidosSelector (: activo) informa falso cuando es verdadero (IE9 / Opera 11.5)
	rbuggyMatches = [];
	...
	// Entonces, permitimos: focus pasar a través de QSA todo el tiempo para evitar el error de IE
	...
	if ((support.qsa = rnative.test (document.querySelectorAll))) {
		...
		// Estrategia de expresiones regulares adoptada de Diego Perini
		afirmar (función (el) {
			...
			// IE 11 / Edge no encuentra elementos en una consulta `[nombre = '']` en algunos casos.
			...
			// Error en la página `selector # id sibling-combinator selector`
			if (! el.querySelectorAll ("a #" + expando + "+ *") .length) {
				rbuggyQSA.push (". #. + [+ ~]");
			}

			// Soporte: Firefox <= 3.6 - 5 solo
			// El viejo Firefox no arroja un identificador mal escapado.
		...
		afirmar (función (el) {
			el.innerHTML = "<a href='' disabled='disabled'> </a>" +
				"<select disabled = 'disabled'> <opción /> </select>";
			...
			// Soporte: IE8
			// Aplicar la distinción entre mayúsculas y minúsculas del atributo de nombre
			if (el.querySelectorAll ("[nombre = d]") .length) {
				rbuggyQSA.push ("nombre" + espacio en blanco + "* [* ^ $ |! ~]? =");
			}

			// FF 3.5 -: enabled /: disabled y elementos ocultos (los elementos ocultos aún están habilitados)
			// IE8 arroja un error aquí y no verá pruebas posteriores
			...
			// Soporte: IE9-11 +
			// IE: el selector deshabilitado no recoge los elementos secundarios de los conjuntos de campos deshabilitados
			...
			// Opera 10-11 no lanza pseudos inválidos posteriores a la coma
	...
	if ((support.matchesSelector = rnative.test ((coincide con = docElem.matches ||
		...
		afirmar (función (el) {
			...
			// Gecko no da error, en su lugar devuelve falso
			coincide con.call (el, "[s! = '']: x");
	...
	función (a, b) {
		...
		// Marca para eliminar duplicados
		si (a === b) {
			hasDuplicate = verdadero;
			devuelve 0;
		}
	...
	función (a, b) {
		...
		// Salga temprano si los nodos son idénticos
		si (a === b) {
			hasDuplicate = verdadero;
			devuelve 0;
		}
		...
		volver yo?
			...
			// Haga una verificación entre hermanos si los nodos tienen un ancestro común
			siblingCheck (ap [i], bp [i]):

			// De lo contrario, los nodos de nuestro documento se ordenan primero
			// Soporte: IE 11+, Edge 17-18+
			// IE / Edge a veces arroja un error de "Permiso denegado" cuando se compara estrictamente
			// dos documentos; Las comparaciones superficiales funcionan.
			/ * eslint-disable eqeqeq * /
...
Sizzle.matchesSelector = function (elem, expr) {
	...
	if (support.matchesSelector && documentIsHTML &&
		...
		tratar {
			...
			// IE 9's matchSelector devuelve falso en los nodos desconectados
			...
			nonnativeSelectorCache (expr, verdadero);
		}
	}
...
Sizzle.contains = function (context, elem) {
	...
	// Establecer los vars del documento si es necesario
	// Soporte: IE 11+, Edge 17-18+
	// IE / Edge a veces arroja un error de "Permiso denegado" cuando se compara estrictamente
	// dos documentos; Las comparaciones superficiales funcionan.
	// eslint-disable-next-line eqeqeq
